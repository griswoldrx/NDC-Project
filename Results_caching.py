...
def get_RXCUI_info(search_results, test_ndc, rxcui_cache_diction, rxcui_cache_fname):
	...
	if status == 'Alien' or status == 'Unknown':
		...
		try:
			...
			drug_name = "Unknown Drug"
	if rxcui not in rxcui_saved_cache:
...
for ndc in full_row_list[1:]:
	...
	if len(test_ndc) == 10:
		test_ndc = "0"+ str(test_ndc)
		print("trying 10 digit NDC padding", test_ndc)
	elif len(test_ndc) == 9:
		test_ndc = "00"+ str(test_ndc)
		print("trying 9 digit NDC padding", test_ndc)
	elif len(test_ndc) == 8:
		test_ndc = "000" + str(test_ndc)
		print("trying 8 digit NDC padding", test_ndc)
	elif len(test_ndc) == 7:
		test_ndc = "0000" + str(test_ndc)
		print("trying 7 digit NDC padding", test_ndc)
	else:
		test_ndc = test_ndc
