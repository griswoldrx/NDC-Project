...
try:
	...
	for ndc in ndcfile:
		target_ndc_list.append(int(ndc.strip()))
except:
...
updated_spreadsheet[0].append('Target Drug?')
...
print("\nUsed %s as the target file when analyzing NDC numbers which contains %s NDC numbers" %(ndcfilename, len(target_ndc_list)))
print("\n\nLength of updated spreadsheet: ", (len(updated_spreadsheet)-1))
print("The number of claims/NDCs that matched your target medication group was: %s " %(flagged))
match_rate = str(float(((count-fail)-1)/(count-1)) *100)
...
spreadsheet_filename = fname + '_flagged_results.csv'
